...
console.warn("unknown message got: %o", arguments);
			...
			createChild: function(name){
				...
				result.removeEventListener = fdmApp.removeEventListener;
				...
				if (!this._listeners.hasOwnProperty(name)){
					...
					this[name] = function(){
						for(var i=0;i<this.length; ++i){
							...
							if (!this[i]) {
								console.error(Object.keys(this), i, name, listener, args, this[i]);
							}
				...
				this._listeners[name].push(listener);
                     ...
                     removeEventListener: function(name, listener){
                         for (var key in this._listeners[name]) {
                             if (this._listeners[name][key] === listener) {
...
//			onThumbnailGenerated: fdm.Events.MultiHandler.create // create manual handler
       ...
       function createHttpRequests() {
           var httpRequests = fdmApp.createChild("httpRequests");
           httpRequests.registerMethod("create", true);
           httpRequests.registerMethod("start", true);
           httpRequests.registerMethod("succeeded", true);
           httpRequests.registerMethod("data", true);
           httpRequests.registerMethod("close", true);
           httpRequests.registerMethod("setCookies", true);
           httpRequests.registerMethod("cookies", true);

           httpRequests.registerEvent("onFinished");

           fdmApp.httpRequests = httpRequests;
       }
    ...
    downloadWizard.registerMethod("makeHttpRequest", true);
              downloadWizard.registerMethod("startUiTransaction", true);
	...
	function createSystem() {
		...
		system.registerEvent("showFFExtensionUpdater");
		system.registerEvent("showFfExtensionUpdater");
		system.registerMethod("removeLegacyFfExtension");

		system.registerEvent("onJsonTaskFromWebExtension");
		system.registerMethod("activateMainWindow");
	}
	...
	function createSettings() {
		...
		settings.registerProperty("show-special-offers");
	...
	function createYtParser() {
		var ytParser = fdmApp.createChild("ytParser");
        ytParser.registerEvent("parseVideo");
        ytParser.registerMethod("onFinished");
	}
