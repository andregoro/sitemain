...
var Schedule = React.createClass({displayName: "Schedule",
    ...
    getInitialState: function () {
        ...
        state.intervalsListMaxHeight = 140;
    ...
    toggleSelect: function(name){
        ...
        if (name == 'start'){
            ...
            if (this.state.timeStartState == this.selectStates.select){
                ...
                let max_height = this.state.intervalsListMaxHeight;
                const dom_node = ReactDOM.findDOMNode(this);
                if (dom_node) {
                    const start_el = document.getElementsByClassName('js_intervals_start');
                    if (start_el && start_el.length) {
                        const start_rect = start_el[0].getBoundingClientRect();
                        max_height = Math.min(window.innerHeight - start_rect.top - start_rect.height - 10, 180);
                    }
                }

                this.setState({
                    timeStartState: this.selectStates.select,
                    intervalsListMaxHeight: max_height
                });
            ...
            if (this.state.timeEndState == this.selectStates.select){
                ...
                const dom_node = ReactDOM.findDOMNode(this);
                if (dom_node) {
                    const start_el = document.getElementsByClassName('js_intervals_end');
                    if (start_el && start_el.length) {
                        const start_rect = start_el[0].getBoundingClientRect();
                        max_height = Math.min(window.innerHeight - start_rect.top - start_rect.height - 10, 180);
                    }
                ...
                React.createElement("div", {className: rjs_class({
                    ...
                    React.createElement("div", {className: "choose"}, 
                        ...
                        React.createElement("div", {className: rjs_class({
                            ...
                            React.createElement("div", {className: "list", style: {display: this.state.timeStartState != this.selectStates.closed ? 'block' : 'none', maxHeight: this.state.intervalsListMaxHeight + 'px'}}, 
                    ...
                    React.createElement("div", {className: "choose"}, 
                        ...
                        React.createElement("div", {className: rjs_class({
                            ...
                            React.createElement("div", {className: "list", style: {display: this.state.timeEndState != this.selectStates.closed ? 'block' : 'none', maxHeight: this.state.intervalsListMaxHeight + 'px'}}, 
