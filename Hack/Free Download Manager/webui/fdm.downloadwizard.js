...
fdm.controllers.DownloadWizard = (function () {
    ...
    function Class(apiWizard, apiSystem) {
	    ...
	    fdm.models.NewDownload = Backbone.Model.extend({
            defaults: {
                ...
                youtubeChannelNextPageUrl: '',
                youtubeChannelCookies: '',         
                VideoQualityForChannel: 'High (1080p)', //default quality
                youtubeChannelAddDateInFilename: true,
                youtubeChannelCountVideoChecked: 0,
                loadMoreVideosRunning: false,
                loadMoreVideosError: false,

                youtubePlaylistDownload: 0,
            },
        ...
        _.bindAll(this, 'onRequestDownloadInfoSuccess', 'onRequestDownloadInfoFail', 'onRequestDownloadBaseInfoGot', 'onValidateFolderResult',
			...
			'addListLinks', 'applyFilePreview', 'onDefaultTimetableChanged', 'createYoutubeSourceCallback', 'updateYoutubeVideosList');
	    ...
	    this.model.on("change:youtubeChannelVideos", this.onChangeYoutubeChannelVideos, this);
    ...
    Class.prototype = {
		...
		onApiOpen: function(url, core_int, youtubeListVideosDownload){

            youtubeListVideosDownload = parseInt(youtubeListVideosDownload) || 0;
            let youtubeChannelVideosDownload = youtubeListVideosDownload & 1;
            let youtubePlaylistDownload = youtubeListVideosDownload & 2;
            let youtubeVideoDownload = youtubeListVideosDownload & 4;
            let youtubePlaylistOnVideoPage = false;
            if (!youtubeVideoDownload && !youtubeChannelVideosDownload && !youtubePlaylistDownload) {
                if (fdm.urlUtils.isYoutubePlaylistOnVideoPage(url)) {
                    youtubePlaylistDownload = true;
                    youtubePlaylistOnVideoPage = true;
                }
                if (fdm.urlUtils.isYoutubePlaylistPage(url)) {
                    youtubePlaylistDownload = true;
                }
            }
			...
			this.newDownload(url, core_int, youtubeListVideosDownload);

			if (!youtubePlaylistOnVideoPage) {
                if (youtubeChannelVideosDownload) {
                    this.createYoutubeSource(core_int);
                } else if (youtubePlaylistDownload) {
                    this.createYtPlaylistSource(core_int);
                } else {
		...
		newDownload: function(rawSource, core_int, youtubeListVideosDownload){
            ...
            let youtubeVideoDownload = youtubeListVideosDownload & 4;
            if (!youtubeVideoDownload && !youtubeChannelVideosDownload && !youtubePlaylistDownload) {
                if (fdm.urlUtils.isYoutubePlaylistPage(rawSource)) {
                    youtubePlaylistDownload = true;
                } else if (fdm.urlUtils.isYoutubeChannelVideosUrl(rawSource))  {
                    youtubeChannelVideosDownload = true;
                }
            }
            ...
            this.model.set({
                ...
                targetFolderIsValid: true,
                youtubePlaylistDownload: youtubePlaylistDownload,
                youtubeChannelVideosDownload: youtubeChannelVideosDownload
            });
        ...
        applySource: function () {
            ...
            if (this.model.get('requestingDownloadInfo')) {
                ...
                if (this.model.get('youtubeChannelVideosDownload')) {
                    this.createYoutubeSource();
                } else if (this.model.get('youtubePlaylistDownload')) {
                    this.createYtPlaylistSource();
                } else {
                    this.createSource();
                }
            }
        ...
        createYoutubeSource: function (core_int) {

            var rawSource = this.model.get('rawSource');
            youtubeChannelParser.getVideosInfoPage(rawSource, '', this.createYoutubeSourceCallback);
        ...
        createYtPlaylistSource: function (core_int) {

            var rawSource = this.model.get('rawSource');
            app.controllers.ytParser.parseVideoPlaylist(rawSource)
				.then(function (res) {
                    this.createYoutubeSourceCallback({videos: res.videos, channel_info: res.channel_info}, res.cookies, res.url_more);
                }.bind(this))
				.catch(function(e){
                    ...
                    var changes = {
                        downloadInfoErrorMessage: error_msg,
                        requestingDownloadInfo: false
                    };
            ...
            this.model.set({
                requestingDownloadInfo: true,
                youtubeChannelAddDateInFilename: false
            });
        },

        createYoutubeSourceCallback: function (youtube_videos, cookies_new, nextPageUrl) {

            if (youtubeChannelParser.stop_createYoutubeVideosList) {
                youtubeChannelParser.stop_createYoutubeVideosList = false;
            ...
            var model_data = {};
            model_data.addSourcePageIsShown = false;
            model_data.sourceInfoPageIsShown = true;
            model_data.requestingDownloadInfo = false;
            model_data.youtubeChannelVideos = youtube_videos;
            model_data.youtubeChannelCookies = cookies_new;
            model_data.youtubeChannelNextPageUrl = nextPageUrl;
            let lastFolders = _apiWizard.getLastFolders();
            if (typeof lastFolders === 'string')
                lastFolders = [lastFolders];
            model_data.lastFolders = lastFolders;
            model_data.targetFolder = lastFolders[0];
            model_data.name = youtube_videos.channel_info.name; //directory to save video files            
            model_data.add_video_date_to_file = false;
            model_data.source = this.model.get('rawSource');
            // model_data.targetFolder = downloadInfo.autoDetectedFolder;
        ...
        tryLoadingVideosAgain: function() {
            this.model.set({loadMoreVideosError: false});
            this.ytLoadMoreVideos();
		},

        ytLoadMoreVideos: function() {
            if (this.model.get('loadMoreVideosRunning') || this.model.get('loadMoreVideosError')) { // prevent parallel run
                return;
            }

            this.model.set({loadMoreVideosRunning: true, loadMoreVideosError: false});

            const current_link = app.controllers.downloadWizard.model.get('source');
            if (this.model.get('youtubePlaylistDownload')) {
                app.controllers.ytParser.paginationVideoPlaylist(this.model.get('youtubeChannelNextPageUrl'), this.model.get('youtubeChannelCookies'))
                    .then(function (res) {
                        this.updateYoutubeVideosList({videos: res.videos, channel_info: res.channel_info}, res.cookies, res.url_more);
                    }.bind(this))
                    .catch(function(e){
                    	if (!this.model.get('sourceInfoPageIsShown') || current_link !== app.controllers.downloadWizard.model.get('source')) {
                    		return;
						}
                        let error_msg = "Unexpected error";
                        if (e && e.hasOwnProperty('message')) {
                        ...
                        var changes = {
                            loadMoreVideosError: error_msg,
                            loadMoreVideosRunning: false
                        };
                        this.model.set(changes);
                    }.bind(this));
            } else {
                youtubeChannelParser.getVideosInfoPage(
                    this.model.get('youtubeChannelNextPageUrl'),
                    this.model.get('youtubeChannelCookies'),
                    function (youtube_videos, cookies_new, nextPageUrl) {
                        if (!this.model.get('sourceInfoPageIsShown') || current_link !== app.controllers.downloadWizard.model.get('source')) {
                            return;
                        }
                        this.updateYoutubeVideosList(youtube_videos, cookies_new, nextPageUrl);
                    }.bind(this)
                );
            }
		},

        updateYoutubeVideosList: function(youtube_videos, cookies_new, nextPageUrl) {
            // if (youtubeChannelParser.stop_updateYoutubeVideosList) {
            //     youtubeChannelParser.stop_updateYoutubeVideosList = false;
            //     return;
            // }

            //add videos to list
            this.model.attributes.youtubeChannelVideos.videos =
                this.model.attributes.youtubeChannelVideos.videos.concat(youtube_videos.videos);
            //update cookies
            this.model.attributes.youtubeChannelCookies = cookies_new;
            //update nextPageUrl
            this.model.attributes.youtubeChannelNextPageUrl = nextPageUrl;
            //update state
            // this.setState(app.controllers.downloadWizard.model.toJSON());

            //enlarge height of videos list when no more videos to add
            // if (nextPageUrl.length === 0) {
            //     $('div.popup').addClass('no_load_more');
            // }

            this.model.set({loadMoreVideosRunning: false, loadMoreVideosError: false});
		},
		...
		validateFolderPromise: function(){
            ...
            return new Promise(function(resolve, reject) {
				...
				if (folder === null)
					resolve([false, '']);
				else if (folder === "" && name === "" && !is_name_fake)
					resolve([false, __('Selected file name and file path are invalid.')]);
				else if (folder === "")
					resolve([false, __('Selected file path is invalid.')]);
				else{
console.log('validateFolderPromise CALL');                    
console.log('folder: ' + folder);
console.log('name: ' + name);
console.log('is_name_fake: ' + is_name_fake);
					_apiSystem.validateFolder(folder, name, false, is_name_fake, function(serialized){
console.log('serialized - below');                        
console.log(serialized);                        
        ...
        calcSizeOnDiskForSelectionPromise: function () {
            ...
            return new Promise(function(resolve, reject) {
// console.error('calcSizeOnDiskForSelection CALL');
...
console.log('ids: ' + ids);
		...
		onChangeYoutubeChannelVideos: function(){
            const s_ids = this.getCheckedYoutubeChannelUrlsIndexes();
			this.model.set({
                youtubeChannelCountVideoChecked: s_ids.length
			});
			this.setValidateAllTimeout();
		},
		...
		createDownloadAfterCheck: function(){
			...
			var time = (+ new Date);
            //For Youtube Channel get task id from core.
            if (this.model.get('youtubeChannelVideosDownload') || this.model.get('youtubePlaylistDownload')) {
                var task_id = this.model.get('id');
                if (task_id === this.model.defaults.id) {
                    task_id = _apiWizard.startUiTransaction();
                    this.model.set("id", task_id);
				}
            }
		...
		validateAllPromise: function(){
            if ( (this.model.get('youtubeChannelVideosDownload') || this.model.get('youtubePlaylistDownload')) && this.model.get('id') === -1 ) {
                return this.validateFolderPromise()
                    .then(this.onValidateFolderResult.bind(this))
		...
		setStartable: function(){
			...
			if (startable && this.model.get("listLinksIsShown")){
				...
				if (!selected_urls || !selected_urls.length) {
                    startable = false;
                    batchDownloadErrorMessage = __('Please select download link');
                }
			}

			if (startable && (this.model.get("youtubeChannelVideosDownload") || this.model.get("youtubePlaylistDownload"))) {
                const s_ids = this.getCheckedYoutubeChannelUrlsIndexes();
                if (!s_ids || !s_ids.length) {
                    startable = false;
                    batchDownloadErrorMessage = __('Please select video');
                }
            }

			this.model.set({
                startable: startable,
                batchDownloadErrorMessage: batchDownloadErrorMessage
			});
		},
        
        prepareYoutubeChannelCheckedUrlsData: function() {
            var videos_prepared = [];            
            var videos_info = this.model.get("youtubeChannelVideos");
            
            for (var key in videos_info.videos) {
                if (videos_info.videos[key].checked) {
                    videos_prepared.push({
                        checked: true, //ignored in core?
                        data: {
                            index: parseInt(key), //ignored in core?
                            name: videos_info.videos[key].title,
                            url: videos_info.videos[key].video_url,
                        },
                        id: 0 //ignored in core?
                    });
                }
            }
            
            return videos_prepared;
        },
        
        getCheckedYoutubeChannelUrlsIndexes: function() {
            var selected_ids = [];            
            var videos_info = this.model.get("youtubeChannelVideos");
        ...
        createDownload: function () {
            ...
            //Create Youtube Channel Videos Download --- BEGIN -----------------------
            if (this.model.get("youtubeChannelVideosDownload") || this.model.get("youtubePlaylistDownload")) {
                var downloadInfo = {
                    autoDetectedFolder: this.model.get("targetFolder"),
                    files: this.prepareYoutubeChannelCheckedUrlsData(),
                    selectedIds: this.getCheckedYoutubeChannelUrlsIndexes(),
                    name: name,
                    filesSelectRule: fdm.models.VideoQualityOptionsToVideoFormat[this.model.get("VideoQualityForChannel")],
                    addDateInFilename:  this.model.get("youtubeChannelAddDateInFilename"),
                    allDownloadsUrl:  this.model.get("source"),
            ...
            //Create Youtube Channel Videos Download --- END -----------------------
	    ...
	    onCreateDownloadSuccess: function (/*successFn*/) {
		    var dataModel = this.model;
//console.log('before dataModel.set');    

        	    this.model.off("change:targetFolder", this.onChangeTargetFolder, this);
                   this.model.off("change:name change:createSubDirectory", this.onChangeName, this);
...
//console.log('after dataModel.set');            
		    //successFn();
			this.close();
	    },

		selectByDragDrop: function (url) {

            let youtubeChannelVideosDownload = fdm.urlUtils.isYoutubeChannelVideosUrl(url);
            let youtubePlaylistDownload = false;
            let youtubePlaylistOnVideoPage = false;

            if (!youtubeChannelVideosDownload && !youtubePlaylistDownload) {
                if (fdm.urlUtils.isYoutubePlaylistOnVideoPage(url)) {
                    youtubePlaylistDownload = true;
                    youtubePlaylistOnVideoPage = true;
                }
        ...
        */
		...
		handleKeydown: function (e) {
			...
			if (this.model.get('sourceInfoPageIsShown')
				...
				stopEventBubble(e);
				if (this.model.get('requestingDownloadInfo') &&
					(this.model.get('youtubeChannelVideosDownload') || this.model.get('youtubePlaylistDownload'))) {
					return;
				}
				this.applySource();
			}
